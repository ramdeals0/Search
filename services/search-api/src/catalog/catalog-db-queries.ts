...
export async function fetchSearchCandidatesFromDatabase(input: {
  ...
  const ftsVector = `to_tsvector('english', coalesce(p.title, '') || ' ' || coalesce(p.description, ''))`;
  const sql = `
    SELECT p.id
    FROM "Product" p
    ...
    WHERE ${clause}
      AND (
        ${ftsVector} @@ plainto_tsquery('english', $1)
        OR p.title ILIKE $1 || '%'
        OR b.name ILIKE $1 || '%'
        OR c.department ILIKE $1 || '%'
      )
    ORDER BY ts_rank(${ftsVector}, plainto_tsquery('english', $1)) DESC,
    p."inStock" DESC,
    p.title ASC
    LIMIT ${limit}
  `;
